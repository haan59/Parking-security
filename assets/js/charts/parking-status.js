...
function drawDotMatrix(canvas, percent) {
    ...
    const matrixWidth = Math.min(cssWidth, 124);
    ...
    const offsetX = (cssWidth - matrixWidth) / 2;
    const offsetY = (cssHeight - matrixHeight) / 2;
    const leftPadding = Math.max(6, Math.round(cssWidth * 0.04));
    const startX = leftPadding + radius;
